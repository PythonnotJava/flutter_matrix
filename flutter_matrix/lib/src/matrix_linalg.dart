...
extension MatrixLinalg on Matrix {
  /// Transpose a matrix
  Matrix transpose() =>
      Matrix.fromList(_transpose(mt_self: self, mt_shape: shape),
          known_row: shape[1], known_column: shape[0]);
  ...
  Matrix dot({required Matrix other}) {
    ...
    final data =
        List.generate(row, (i) => List.filled(column, 0.0, growable: true));
    for (var i = 0; i < row; i++) {
  ...
  Matrix kronecker({required Matrix other}) {
    ...
    return Matrix.fromList(data,
        known_row: row * other_row, known_column: column * other_column);
  }
  ...
  Matrix get adjugate {
    ...
    for (int r = 0; r < n; r++) {
      for (int c = 0; c < n; c++) {
        final cof = _coincidental_list(
            row: r, column: c, mt_self: self, mt_shape: [n, n]);
        data[c][r] = ((r + c) % 2 == 0 ? 1 : -1) *
            _det(mt_shape: [n - 1, n - 1], mt_self: cof);
      }
    }
  ...
  Matrix get rref => Matrix.fromList(_rref(mt_self: self, mt_shape: shape),
      known_row: shape[0], known_column: shape[1]);
