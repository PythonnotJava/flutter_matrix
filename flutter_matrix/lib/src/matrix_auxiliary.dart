...
mixin MatrixAuxiliary {
  ...
  double _pow_double(double base, double exponent) =>
  ...
  double Function(double, double) _math_basement_double(int mode) {
    final double Function(double, double) func =
        switch (mode) { 0 => _pow_double, 1 => math.atan2, _ => _pow_double };
    return func;
  }
  ...
  double _det(
      {required List<List<double>> mt_self, required List<int> mt_shape}) {
    var [row, column] = mt_shape;
    assert(row == column);
    double detValue = 1.0;
    int n = row;
    final matrixcpy =
        mt_self.map((row_list) => row_list.map((e) => e).toList()).toList();
  ...
  /// Transpose of a list
  List<List<double>> _transpose(
      {required List<List<double>> mt_self, required List<int> mt_shape}) {
    var [row, column] = mt_shape;
    return List.generate(
        column, (r) => List.generate(row, (c) => mt_self[c][r]));
  }
  ...
  List<Complex> _fft(
    ...
    List<Complex> even =
        _fft(mt_self: _selectEveryNth(mt_self, 2, 0), mt_shape: [row ~/ 2, 2]);
    List<Complex> odd =
        _fft(mt_self: _selectEveryNth(mt_self, 2, 1), mt_shape: [row ~/ 2, 2]);
