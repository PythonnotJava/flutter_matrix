...
extension MatrixBase on Matrix {
  ...
  bool isSameObj(Matrix other) =>
      identical(this, other) && identical(self, other.self);
  ...
  Matrix _abstract_operator_method(int mode,
      {Matrix? other, double? number, int dim = -1}) {
    ...
    if (other != null) {
      ...
      if (dim == 0) {
        ...
        data = List.generate(row,
            (r) => List.generate(column, (c) => func(self[r][c], other[0][c])));
      } else if (dim == 1) {
        assert(row == other_row && other_column == 1);
        data = List.generate(row,
            (r) => List.generate(column, (c) => func(self[r][c], other[r][0])));
      } else {
        assert(row == other_row && column == other_column);
        data = List.generate(row,
            (r) => List.generate(column, (c) => func(self[r][c], other[r][c])));
      }
    } else {
      number!;
      data = List.generate(
          row, (r) => List.generate(column, (c) => func(self[r][c], number)));
    }
  ...
  Matrix add({Matrix? other, double? number, int dim = -1}) =>
      _abstract_operator_method(0, other: other, number: number, dim: dim);

  /// Subtraction.
  Matrix minus({Matrix? other, double? number, int dim = -1}) =>
      _abstract_operator_method(1, other: other, number: number, dim: dim);

  /// Multiplication.
  Matrix multiply({Matrix? other, double? number, int dim = -1}) =>
      _abstract_operator_method(2, other: other, number: number, dim: dim);

  /// Division
  Matrix divide({Matrix? other, double? number, int dim = -1}) =>
      _abstract_operator_method(3, other: other, number: number, dim: dim);
  ...
  Matrix concat({required Matrix other, bool horizontal = true}) {
    ...
    if (horizontal) {
      ...
      return Matrix.fromList(data,
          known_row: row, known_column: column + other_column);
    } else {
      ...
      return Matrix.fromList(data,
          known_row: row + other_row, known_column: column);
    }
  }
  ...
  Matrix flatten({bool horizontal = true}) {
    if (horizontal) {
      return Matrix.fromList([self.expand((e) => e).toList()],
          known_row: 1, known_column: size);
    } else {
      int row = shape[0];
      final data = [
        List.generate(size, (index) => self[index % row][index ~/ row])
      ];
  ...
  Matrix slice({required int start, int? end, bool horizontal = true}) {
    ...
    if (horizontal) {
      ...
      return Matrix.fromList(data,
          known_column: (start - end).abs() + 1, known_row: row);
    }
  }
  ...
  Matrix select({required List<int> target, bool horizontal = true}) {
    ...
    if (horizontal) {
      ...
      return Matrix.fromList(List.generate(target_len, (r) => row_(target[r])),
          known_row: target_len, known_column: column);
    } else {
      ...
      return Matrix.fromList(
          List.generate(
              row,
              (r) =>
                  List<double>.generate(target_len, (c) => self[r][target[c]])),
          known_row: row,
    ...
    switch (which) {
      case 0:
        return List.generate(row,
            (r) => BoolList.generate(column, (c) => self[r][c] > other[r][c]));
      case 1:
        return List.generate(row,
            (r) => BoolList.generate(column, (c) => self[r][c] < other[r][c]));
      case 2:
  ...
  List<dynamic> toList(Typed T) {
    switch (T) {
      case Typed.int:
        return self
            .map((rowList) => rowList.map((e) => e.toInt()).toList())
            .toList();
      case Typed.double:
        return self.map((rowList) => rowList.toList()).toList();
      case Typed.bool:
        return self
            .map((rowList) => rowList.map((e) => e != 0).toList())
            .toList();
      case Typed.float32:
      ...
      case Typed.int8:
        return self
            .map((rowList) =>
                Int8List.fromList(rowList.map((e) => e.toInt()).toList()))
            .toList();
      case Typed.int16:
        return self
            .map((rowList) =>
