...
extension MatrixFunctools on Matrix {
  ...
  Object reduce(double Function(double, double) condition,
    ...
    if (dim == 0) {
      ...
      if (element == null) {
        return List.generate(row, (r) => self[r].reduce(condition))
            .reduce(condition);
      }
      var v = self[0][0];
      self[0][0] = condition(element, v);
      final result = List.generate(row, (r) => self[r].reduce(condition))
          .reduce(condition);
      self[0][0] = v;
      return result;
    }
  ...
  Matrix customize(double Function(double) condition) {
    ...
    return Matrix.fromList(
        self
            .map((row_list) => row_list.map((e) => condition(e)).toList())
            .toList(),
        known_column: column,
        known_row: row);
  }
  ...
  Matrix confront(double Function(double, double) condition,
      {required Matrix other}) {
    ...
    return Matrix.fromList(
        List.generate(
            ...
            (r) => List<double>.generate(
                column, (c) => condition(self[r][c], other.self[r][c]))),
        known_row: row,
        known_column: column);
  }
  ...
  Matrix clip(double Function(double) condition,
